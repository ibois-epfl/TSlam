...
//        flags |= CALIB_FIX_PRINCIPAL_POINT;
    ...
    for(i = 0;;i++)
    {
       ...
       // improve the found corners' coordinate accuracy
        ...
        cv::Mat imResized;
        cv::resize(view,imResized,cv::Size(1280,920));
        imshow("Image View", imResized);
    ...
    {
        Mat view, rview, map1, map2,cameraMatrix2;
        cameraMatrix.convertTo(cameraMatrix2,CV_32F);/*
        cameraMatrix2.at<float>(0,0)*=0.8;
        cameraMatrix2.at<float>(1,1)*=0.8;*/
        fisheye::initUndistortRectifyMap(cameraMatrix, distCoeffs, cv::Matx33d::eye(),
                                cameraMatrix2,
         ...
         for( i = 0; i < (int)imageList.size(); i++ )
        {
            ...
            cv::Mat imResized;
            cv::resize(view,imResized,cv::Size(1280,920));
            imshow("Org Image View", imResized);
            cv::resize(rview,imResized,cv::Size(1280,920));
            imshow("Image View", imResized);
            std::cout<<"image "<<imageList[i]<<std::endl;
            char c = (char)waitKey();
            if( c == 27 || c == 'q' || c == 'Q' )
                break;
        }
