...
cv::Mat System::process(const Frame &inputFrame) {
  ...
  if(_17450466964482625197 == MODE_SLAM && !_2869602498954317713->hasMap ())
    _2869602498954317713 -> setParams (TheMap,_14938569619851839146.enableLoopClosure);
    ...
    if(TheMap -> isEmpty ()  &&  _17450466964482625197 ==  MODE_SLAM )  {
      ...
      if(_3857178690860967008 == STATE_TRACKING ) {
        currentKeyFrameId_105767 = _1513765969352381626 (_4913157516830781457,_17976495724303689842 );
        // bug here
        _17976495724303689842 = _11166622111371682966 ( frame_149385,_17976495724303689842 );
...
STATE_LOST ;
    ...
    STATE_LOST )
 {
   ...
   (
    ...
    frame_149385,_5564636146947005941 )
...
_5564636146947005941 ;
         ...
         _1513765969352381626 ( frame_149385,_17976495724303689842 )
        ;
                _10558050725520398793 =

 frame_149385.fseq_idx ;
...
newFrame ( frame_149385,currentKeyFrameId_105767 )
...
STATE_LOST &&
 ...
 ;
  ...
  TheMap,_14938569619851839146.enableLoopClosure )
...
cv::Mat System ::process (cv::Mat &_6441194614667703750, const ImageParams &_18212413899834346676,
                          uint32_t _9933887380370137445, const cv::Mat & _46082575014988268, const cv::Mat &_1705635550657133790 ) {
  swap (_4913157516830781457,frame_149385);
  std::thread _1403653089436386197;
  if( _17450466964482625197 == MODE_SLAM )
  _1403653089436386197 = std::thread ([&]() {
    if( _2869602498954317713 -> mapUpdate() ) {
      if(_2869602498954317713 -> bigChange ()){
        frame_149385.pose_f2g = _2869602498954317713 -> getLastAddedKFPose ();
        _17976495724303689842 = _2869602498954317713 -> getLastAddedKFPose ();
      }
    };
  });
 ...
 _6441194614667703750,"\x49\x6d\x67\x2e\x53\x69\x7a\x65\x3a"+_5829441678613027716 (
    ...
    frame_149385.imageParams.CamSize.width )
     +"\x78"+_5829441678613027716 (
       ...
       frame_149385.imageParams.CamSize.height )
...

 {
  ...
  _2869602498954317713 ->
   ...
   bigChange ()
                ...
                frame_149385.pose_f2g = _2869602498954317713 ->
...
IPPE ::
 ...
 _14938569619851839146.aruco_markerSize,_2654435878.und_corners,inputFrame.imageParams.CameraMatrix,inputFrame.imageParams.Distorsion )
...
uint32_t System::_1513765969352381626 (const Frame &_10614055813325501077, const se3 &_10706799483371532009) {
  int64_t _644211886852851457 = -1 ;

  if (_14938569619851839146.detectKeyPoints )
    _644211886852851457 = TheMap -> getReferenceKeyFrame (_10614055813325501077,1 );

  if (_644211886852851457 != -1 )
    return _644211886852851457 ;
  ...
  for( auto _2654435878:_10614055813325501077.markers ) {
  ...
  pair <uint32_t,float > _18337238202410394478 ( std ::numeric_limits<uint32_t >::max (), std::numeric_limits<float>::max() );
  ...
  for(auto _3005399795337363304:_4240713669852012646 )
    for( const auto &inputFrame:TheMap->map_markers [_3005399795337363304].frames ) {
      auto _2654435869 = _10706799483371532009.t_dist ( TheMap->keyframes [ inputFrame ].pose_f2g );
...
inputFrame,_13011065492167565582 ) ) return
...
se3 System::_11166622111371682966 (Frame &frame_169403,se3 se3_143874 ) {
  ...
  if ( TheMap ->map_points.size () > 0 ){
    if(!_14463320619150402643.empty ())
      se3_576955 = _14463320619150402643*se3_576955.convert ();

    frame_169403.pose_f2g = se3_576955;
    DMatchVector_637068 = _11946837405316294395 ( frame_169403,_4913157516830781457,_14938569619851839146.maxDescDistance * 1.5, _14938569619851839146.projDistThr );

    int _9870110657242862171 = 0 ;

    if ( DMatchVector_637068.size () > 30 ){
      auto _14671559813105454070 = se3_576955 ;
      _9870110657242862171 = PnPSolver :: solvePnp ( frame_169403,TheMap,DMatchVector_637068,_14671559813105454070,currentKeyFrameId_105767 );
      ...
      if (TheMap->keyframes.is(currentKeyFrameId_105767)) {
        ...
        _16997326787393468537.setParams ( TheMap -> keyframes [ currentKeyFrameId_105767 ], FrameMatcher :: MODE_ASSIGNED,_14938569619851839146.maxDescDistance*2,0.6,true,3 ) ;
    ...
    auto _3521005873836563963 = TheMap -> matchFrameToMapPoints (
      ...
      frame_169403, se3_576955 ,_14938569619851839146.maxDescDistance*2, _3763415994652820314,true );

    DMatchVector_637068.insert ( DMatchVector_637068.end (),_3521005873836563963.begin (),_3521005873836563963.end ()  );

    filter_ambiguous_query (DMatchVector_637068 ) ;

  }

  int _8650310500205049352 = PnPSolver ::solvePnp ( frame_169403,TheMap,DMatchVector_637068, se3_576955, currentKeyFrameId_105767 );
  bool _6535949166237672095 = false ;

  if (frame_169403.markers.size ()>0 ){
    int _9870110666330890270 = 0 ;

    for( size_t _2654435874 =  0 ; _2654435874 <  frame_169403.markers.size () ; _2654435874 ++ ) {
      auto _11093822290813 = TheMap -> map_markers.find (frame_169403.markers [ _2654435874 ] .id );

      if( _11093822290813 ==     TheMap ->  map_markers.end () )
        continue ;

      if ( ! _11093822290813 -> second.pose_g2m.isValid () )
        continue ;
    ...
    if ( _9870110666330890270 > 1 )
      _6535949166237672095 = true ;

  }

  if ( _8650310500205049352 < 30 && !_6535949166237672095 ) {
    return  se3 () ;
  }
...
-_46082575822903876 )
    ...
    ,_9971115036363993554* ( frame_149385.kpts [
       _2654435874 ]
...
&currentKeyFrameId_105767,sizeof (
