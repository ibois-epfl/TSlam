...
#pragma message "warning : in non-sequential mode detected markers in loop closure are not proceesed properly?"
        ...
        if (_LoopClosureInfo.foundLoop()){
            ...
            for(auto mm:TheMap->map_markers)
            __UCOSLAM_TIMER_EVENT__("detectLoop");
...
bool MapManager::mapUpdate(   ){
	...
	//Frames with bad associations
//	std::set<uint32_t> badAss_frames;
//        for(auto badAss:BadAssociations)
//            badAss_frames.insert(TheMap->keyframes[badAss.second].idx);
        //Keyframes without mappoints will be removed
//        for(auto frameId:badAss_frames)
//        {
//            size_t n=0;
//            for(auto id:TheMap->keyframes[frameId].ids)
//            {
//                if(id== std::numeric_limits<uint32_t>::max()) n++;
//            }
//            if(TheMap->keyframes[frameId].ids.size()-n == 0)
//                KeyFramesToRemove.insert(frameId);
//        }
    }
    ...
    //Remove keyframes without mappoints
    for(auto f:TheMap->keyframes)
    {
        bool remove=true;
        for(auto id:TheMap->keyframes[f.idx].ids)
        {
        ...
        if(remove) KeyFramesToRemove.insert(f.idx);
    }
    ...
    for(size_t m=0;m< newFrame.markers.size();m++){
        ...
        if (!map_marker.pose_g2m.isValid() && map_marker.frames.size()>=size_t(System::getParams().aruco_minNumFramesRequired) ){
            ...
            if (farEnoughViews.size()>=size_t(System::getParams().aruco_minNumFramesRequired)){//at least x views to estimate the pose using multiple views
                vector<reslam::MarkerObservation> marker_views;
...
void MapManager::mainFunction(){
  ...
  //  assert(TheMap->checkConsistency(true ));
    ...
    if(!_hurryUp ){
        ...
        {
            if(TheMap->state==TheMap->ORIGINAL)
                //When the map is original mode, it is possible remove frames
                TheMap->keyframes[kf].setBad (true);
            else
            {
                //But, when the map is modified, only it is posible remove new frames
                if(!TheMap->keyframes[kf].isFromMapOriginal())
                    TheMap->keyframes[kf].setBad (true);
            }
        }
    }
...
set<uint32_t> MapManager::keyFrameCulling(uint32_t keyframe_idx) {
    ...
    if( System::getParams().detectMarkers  && TheMap->map_markers.size()!=0 ){//must be in both
        ...
        for(auto kf:KFtoRemove){
            ...
            if(ThisKFrame.markers.size()==0) continue;
...
vector<uint32_t> MapManager::getMatchingFrames(Frame &NewFrame,size_t maxFrames)    {
    ...
    //remove bad ones
    size_t i=0, newMapFrames=0;
    while( i<neighbors.size()){
        if(!TheMap->keyframes[neighbors[i]].isFromMapOriginal())
            newMapFrames++;
        //if ( TheMap->keyframes[neighbors[i]].isBad() || !TheMap->keyframes[neighbors[i]].isMapOriginal()){
...
//    std::cout <<"newMapFrames ("<< newMapFrames<<","<< neighbors.size()<<")"<< std::endl;
...
//     std::cout <<"Neighbors Size"<<neighbors.size()<<std::endl;
     //Remove percentage of new frames, only when the map is in modified mode
     if(TheMap->getMapState()==TheMap->STATE::MODIFIED)
     {
         size_t del=0, j=0;
         //     int max = 0.3f*(neighbors.size()-newMapFrames) ;
         //     if(max<0) max=0;
         int max = (System::getParams().nKFMatcher)*(newMapFrames) ;
//              std::cout << "Max new frame to remove"<<max << std::endl;
         while(max > del && j<neighbors.size())
         {
             if(!TheMap->keyframes[neighbors[j]].isFromMapOriginal())
             {
                 std::swap(neighbors[j],neighbors.back());
                 neighbors.pop_back();
                 del++;
             }
             else
    ...
    vector<uint32_t> vpMapPointMatches = mpCurrentKeyFrame.getMapPoints();
      for(auto tkf: vpTargetKFs){
        ...
        for(auto MpId:vpMapPointMatches){
            ...
            int nPredictedLevel = mpCurrentKeyFrame.predictScale(dist,MP.getMaxDistanceInvariance(), TheMap->_maxOctaveLevel) ;//MP.predictScale( dist,mLogScaleFactor,keyframe.scaleFactors.size());
            float radius = th*pow(System::getParams().scaleFactor,TheMap->_maxOctaveLevel-nPredictedLevel);
            // Search in a radius
            //  float radius = th*keyframe.scaleFactors[nPredictedLevel];
//            int sc= floor(log(System::getParams().maxFocalLength/System::getParams().minFocalLength)/log(System::getParams().scaleFactor) - nPredictedLevel);
//            if(sc<0) sc=0;
//            float radius = th*pow(System::getParams().scaleFactor,sc);
            //float radius = th*pow(System::getParams().scaleFactor,mpCurrentKeyFrame.scaleFactors.size()-nPredictedLevel);
            if (MP.getViewCos(camCenter)<0.98) radius*=1.4f;
            vector<uint32_t> vkpIdx=keyframe.getKeyPointsInRegion(p2d,radius,nPredictedLevel,nPredictedLevel+1);
            //find the best one
    ...
    for(auto &mpid:smap_ids){
        ...
        if( MP.getViewCos(camCenter)<0.5)continue;
        //ok, now projection is safe
        int nPredictedLevel = mpCurrentKeyFrame.predictScale(dist,MP.getMaxDistanceInvariance(),TheMap->_maxOctaveLevel); //MP.predictScale( dist,mLogScaleFactor,mpCurrentKeyFrame.scaleFactors.size());
        const float radius = th*mpCurrentKeyFrame.scaleFactors[TheMap->_maxOctaveLevel-nPredictedLevel];
        // Search in a radius
        //const float radius = th*mpCurrentKeyFrame.scaleFactors[nPredictedLevel];
//        int sc= floor(log(System::getParams().maxFocalLength/System::getParams().minFocalLength)/log(System::getParams().scaleFactor) - nPredictedLevel);
//        if(sc<0) sc=0;
//        float radius = th*pow(System::getParams().scaleFactor,sc);
        //float radius2 = th*pow(System::getParams().scaleFactor,mpCurrentKeyFrame.scaleFactors.size()-nPredictedLevel);
        vector<uint32_t> vkpIdx=mpCurrentKeyFrame.getKeyPointsInRegion(p2d,radius,nPredictedLevel,nPredictedLevel+1);
        //find the best one
        pair<float,int>  best(System::getParams().maxDescDistance+1e-3,-1);
        for(auto kpidx:vkpIdx){
...
std::list<MapManager::NewPointInfo> MapManager::createCloseStereoPoints(Frame & NewFrame ){
    ...
    if(closePoints.size()>reslam::System::getParams().maxNewPoints){
...
//    cout<<";???="<<closePoints[0].depth<<" "<<closePoints.back().depth<<" "<< closePoints.size()<<" "<<reslam::System::getParams().maxNewPoints<<endl;exit(0);
    closePoints.resize( std::min( closePoints.size(),size_t(reslam::System::getParams().maxNewPoints)));
//    if(nclose<reslam::System::getParams().maxNewPoints){
       ...
       vector<cv::DMatch> matches=FMatcher.matchEpipolar(frame2,FrameMatcher::MODE_UNASSIGNED,FQ2T);
        ...
        for(size_t i=0;i<matches.size();i++)
            if (!isnan( p3d[i].x)){
                ...
                const float ratioOctave =frame2.scaleFactors[oct_frame2]/NewFrame.scaleFactors[oct_NewFrame];
...
//    std::cout <<"Matches  ("+_to_string(matchingFrames.size())+")";
    ...
    for(auto &kp:KptIndex_MatchInfo )
    {
        ...
        int bestDesc=-1;
        int bestOctave=std::numeric_limits<int>::min();
