         ...
         for( const auto &f_info:mp.frames)
        {
            ...
            if ( depth<=0){
                ...
                if( Frame.imageParams.isArray()){
                    ...
                    float x = ((kpUn.pt.x - Frame.imageParams.cx(0)) * depth)/Frame.imageParams.fx(0);
                    float y = ((kpUn.pt.y - Frame.imageParams.cy(0)) * depth)/Frame.imageParams.fy(0);
                    ...
                    obs << kpUn.pt.x, kpUn.pt.y, im2_x, im2_y;

                    EdgeMultiCam2_SE3ProjectXYZ* e = new  EdgeMultiCam2_SE3ProjectXYZ();
