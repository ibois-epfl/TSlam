...
enum STATE{STATE_TRACKING,STATE_LOST,STATE_RELOC};
//working modes of the system
enum MODES{MODE_SLAM,MODE_LOCALIZATION};

enum CAMERA{MONOCULAR,STEREO,ARRAY,RGBD};
...
class RESLAM_API DescriptorTypes  {
public:
...
struct RESLAM_API Params{
    ...
    float minFocalLength=200; //Min focal length used in pyramid
    int featuresFirstLevel=140; //Number of features (minFocalLength)
    float featuresFactor=1; //[0-1] Number features factor
    int maxOctave=-1; //Max octave level (Calculated)
    float nKFMatcher=0.3;//[0,1] Percentaje new keyframes not considered in frame matching.
