...
class RESLAM_API ReSlam{
public:

    //If you want to use a different marker detector, send it here.
    ReSlam(std::shared_ptr<MarkerDetector> mdetector={});
    //
    ~ReSlam();
    ...
    void setParams(std::shared_ptr<Map> map, const  reslam::Params &params, const std::string &vocabulary="");
    ...
    void backwardTracker(const cv::Mat pose_f2g,   const std::vector<cv::Mat> &images, const ImageParams &img_params,uint32_t frameseq_idx);
