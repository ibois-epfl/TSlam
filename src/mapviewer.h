...
vector<sgl::Point3> MapDrawer::getMarkerIdPcd(ucoslam::Marker &minfo,float perct=1 )
{
    ...
    int baseline = 0;
    float _markerSize=0;
    for(int i=0;i<minfo.points3d.size();i++)
        for(int j=i+1;j<minfo.points3d.size();j++)
            _markerSize=std::max(_markerSize,float( cv::norm(minfo.points3d[i]-minfo.points3d[j])));
    float markerSize_2 = _markerSize / 2.f;
