...
If you don't have a camera or for some other reason that you can't run the monocular slam, you can follow the instruction to run the minimal example.
### Minimal example:
```bash
cd build/utils
...
```
This runs `monocular_slam.cpp` with the example video.
