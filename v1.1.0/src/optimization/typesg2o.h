...
class  EdgeMultiCam2_SE3ProjectXYZ: public  g2o::BaseBinaryEdge<4, Eigen::Vector4d, VertexSBAPointXYZ, VertexSE3Expmap>{
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  EdgeMultiCam2_SE3ProjectXYZ(){}
 ...
 virtual void linearizeOplus(){
     ...
     _jacobianOplusXi(0,0) = -fx_1*R(0,0)/z+fx_1*x*R(2,0)/z_2;
     _jacobianOplusXi(0,1) = -fx_1*R(0,1)/z+fx_1*x*R(2,1)/z_2;
     _jacobianOplusXi(0,2) = -fx_1*R(0,2)/z+fx_1*x*R(2,2)/z_2;

     _jacobianOplusXi(1,0) = -fy_1*R(1,0)/z+fy_1*y*R(2,0)/z_2;
     _jacobianOplusXi(1,1) = -fy_1*R(1,1)/z+fy_1*y*R(2,1)/z_2;
     _jacobianOplusXi(1,2) = -fy_1*R(1,2)/z+fy_1*y*R(2,2)/z_2;

     _jacobianOplusXi(2,0) = -fx_2*R(0,0)/z2+fx_2*x2*R(2,0)/z2_2;
     _jacobianOplusXi(2,1) = -fx_2*R(0,1)/z2+fx_2*x2*R(2,1)/z2_2;
     _jacobianOplusXi(2,2) = -fx_2*R(0,2)/z2+fx_2*x2*R(2,2)/z2_2;

     _jacobianOplusXi(3,0) = -fy_2*R(1,0)/z2+fy_2*y2*R(2,0)/z2_2;
     _jacobianOplusXi(3,1) = -fy_2*R(1,1)/z2+fy_2*y2*R(2,1)/z2_2;
     _jacobianOplusXi(3,2) = -fy_2*R(1,2)/z2+fy_2*y2*R(2,2)/z2_2;

     _jacobianOplusXj(0,0) =  x*y/z_2 *fx_1;
     _jacobianOplusXj(0,1) = -(1+(x*x/z_2)) *fx_1;
     _jacobianOplusXj(0,2) = y/z *fx_1;
     _jacobianOplusXj(0,3) = -1./z *fx_1;
     _jacobianOplusXj(0,4) = 0;
     _jacobianOplusXj(0,5) = x/z_2 *fx_1;

     _jacobianOplusXj(1,0) = (1+y*y/z_2) *fy_1;
     _jacobianOplusXj(1,1) = -x*y/z_2 *fy_1;
     _jacobianOplusXj(1,2) = -x/z *fy_1;
     _jacobianOplusXj(1,3) = 0;
     _jacobianOplusXj(1,4) = -1./z *fy_1;
     _jacobianOplusXj(1,5) = y/z_2 *fy_1;

     _jacobianOplusXj(2,0) =  x2*y2/z2_2 *fx_2;
     _jacobianOplusXj(2,1) = -(1+(x2*x2/z2_2)) *fx_2;
     _jacobianOplusXj(2,2) = y2/z2 *fx_2;
     _jacobianOplusXj(2,3) = -1./z2 *fx_2;
     _jacobianOplusXj(2,4) = 0;
     _jacobianOplusXj(2,5) = x2/z2_2 *fx_2;

     _jacobianOplusXj(3,0) = (1+y2*y2/z2_2) *fy_2;
     _jacobianOplusXj(3,1) = -x2*y2/z2_2 *fy_2;
     _jacobianOplusXj(3,2) = -x2/z2 *fy_2;
     _jacobianOplusXj(3,3) = 0;
     _jacobianOplusXj(3,4) = -1./z2 *fy_2;
     _jacobianOplusXj(3,5) = y2/z2_2 *fy_2;
 }
...
//     p3d_c1 = (cv::Mat_<float>(4,1) << trans_xyz[0], trans_xyz[1], trans_xyz[2], 1);
//     p3d_c2 = Q2T * p3d_c1;

//     const float invz2 = 1.0/p3d_c2.at<float>(2);
//     res[2] = p3d_c2.at<float>(0)*invz2*fx_2 + cx_2;
//     res[3] = p3d_c2.at<float>(1)*invz2*fy_2 + cy_2;
