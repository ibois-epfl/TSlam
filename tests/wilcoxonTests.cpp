...
#include "logtools.h"
using namespace  std;
using namespace  reslam::logtools;
...
TestResult analyzeTest(TestInfo &ti){
    ...
    TestResult tr;
    tr.ti=ti;
    tr.gt=reslam::logtools::loadFile(ti.gt_file);
    tr.poses=reslam::logtools::loadFile(ti.fullPath);



    tr.matchedFrames =reslam::logtools::getMatchedLocations(tr.gt,tr.poses);
    if (tr.matchedFrames .size()==0) return tr;
    reslam::logtools::alignAndScaleToGroundTruth(tr.matchedFrames );
    //get the ATE error
    double e=0;
   ...
   // reslam::logtools::getMatchedLocations_io(tr.poses,tr.poses);
...
//    cout<<t2res.ATE<<" "<<t2res.perctFramesTracked<<" | ";
    ...
    reslam::logtools::alignAndScaleToGroundTruth(t1res.matchedFrames );
    for(auto &p:t1res.matchedFrames )
        p.error=cv::norm(getT(p.first)-getT(p.second));
    reslam::logtools::alignAndScaleToGroundTruth(t2res.matchedFrames );
