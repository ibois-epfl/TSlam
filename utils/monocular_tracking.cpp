...
void savePosesToFile(string filename, const std::map<uint32_t, tslam::se3> &fmp);
...
int main(int argc,char **argv){
    ...
    tslam::debug::Debug::setLevel( stoi(cml("-debug","0")));
    int waitTime=2;
    ...
    auto TViewer = tslam::MapViewer::create(cml["-noX"] ? "" : "Cv");
    tslam::Slam Slam;
    cv::Size vsize(0,0);
    cv::Mat in_image;
    ...
    Slam.setMode(tslam::Slam::MODE_LOCALIZATION);
    ...
    while(  !finished && !in_image.empty()){
        ...
        Fps.stop();
        int k = TViewer->show(tslam::Map::singleton(), Slam.getCurrentPose_f2g(), Slam.getShowImage(),"#" + std::to_string(currentFrameIndex) + " fps=" + to_string(1./Fps.getAvrg()) );
