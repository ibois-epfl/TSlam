...
//                     std::cout << "dict_info: " << additionalInfo << std::endl;
//                     std::cout << "contourPoints: " << MarkerCanditates[i].contourPoints << std::endl;
...
//                 _candidates_wcontour.erase(std::remove_if(_candidates_wcontour.begin(), _candidates_wcontour.end(), [](const vector<cv::Point2f> &m)
//                                                           { return m.size() == 0; }),
...
//         __ARUCO_TIMER_EVENT__("Corner Refinement");
    ...
    void MarkerDetector_Impl::filter_ambiguous_query(std::vector<cv::DMatch> &matches)
    {
        ...
        if (needRemove)
            matches.erase(std::remove_if(matches.begin(), matches.end(), [](const cv::DMatch &m)
