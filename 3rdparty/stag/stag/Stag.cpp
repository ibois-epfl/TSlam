...
void Stag::logResults(string path)
{
  ...
  if (keepLogs) drawer.drawQuads(path + "5 distorted quads.png", image, quadDetector.getDistortedQuads());
  drawer.drawMarkers(path + "6 markers.png", image, markers);
  if (keepLogs) drawer.drawQuads(path + "7 false quads.png", image, falseCandidates);
