...
int main(int argc, char *argv[]){
    ...
    global2ref_T=tslam::rigidBodyTransformation_Horn1987(orig_markers_corners.at(ref_marker),ref_marker_corners,true);
...
//    hm_ranges=cv::Rect2f(0,0.1,2.1,1.2); grid_step=0.0015;//for hamid's sequences, ref marker 0 (0.108m) theta=PI
    ...
    if(new_cloud_folder.empty()){
        ...
        cv::Mat T=tslam::rigidBodyTransformation_Horn1987(new_marker_points,orig_marker_points,true);
        cout<<T<<endl;
...
//        combination_image=(colorized_nhi+colorized_ohi)/2.0;
        ...
        //    cv::Mat click_T=tslam::rigidBodyTransformation_Horn1987(new_mouse_3D_coords,orig_mouse_3D_coords,true);
        //    //write transform
        //    for(int i=0;i<3;i++)
        //        for(int j=0;j<4;j++)
