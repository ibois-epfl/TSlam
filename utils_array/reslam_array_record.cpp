                 ...
                 if(recording)
                 {
                     for(int c=0;c<ncams;c++)
                     {
                         if(!outfile[c].isOpened())
                            if(!outfile[c].open(outputFolder + "/cam"+std::to_string(c)+".avi", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 15,
                                         cv::Size(Cameras[c].get(cv::CAP_PROP_FRAME_WIDTH),Cameras[c].get(cv::CAP_PROP_FRAME_HEIGHT))))
