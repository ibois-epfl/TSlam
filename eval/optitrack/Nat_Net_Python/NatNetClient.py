...
#Unpack Mocap Data Functions
    ...
    def __data_thread_function(self, in_socket,
        ...
        header = False
        while not stop() and self.__is_recording:
            # Block for input
            try:
            ...
            if len(data) > 0 :
                ...
                # get the true message and data
                message_id, mocap_data = self.__process_message(data , print_level)

                ### Recording
                if mocap_data != None:
                    # we write the csv file
                    with open(csv_file,"a",encoding='utf-8',newline='') as f:
                        # set optitrack frame number
                        current_opti_frame_number = self.opti_frame_number + opti_frame_number
                        csv_writer = csv.writer(f,delimiter=";")
                        if not header:
                            header = mocap_data.get_header()
                            csv_writer.writerow(header)
                            header = True
                        # set optitrack frame number
                        current_time_stamp = time.time() # in milliseconds
                        csv_writer.writerow(mocap_data.get_as_csv(frame_number=current_opti_frame_number,
                                                                  time_stamp=current_time_stamp))
                        opti_frame_number += 1
                
                ### STOP Recording
                data = bytearray(0)
        self.opti_frame_number += opti_frame_number
    ...
    # >>> added >>>
    def get_nbr_frames(self):
        return self.opti_frame_number

    # >>> modified >>>
    def run(self , csv_file, img_folder):
        """ Run the client for recording images and pose stream data """
    ...
    def shutdown(self):
        """ Shutdown the NatNet client and the python process"""
        print("[INFO]: shutdown called")
        print("[INFO]: wait for the program to close all threads and kill the process ...")
        self.__is_recording = False
